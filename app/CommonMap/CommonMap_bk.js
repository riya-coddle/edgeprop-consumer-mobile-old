...
const PAGE_SIZE = 10;
const TIMEOUT = 5000;
const CENTER_LAT_DEFAULT = 1.352083;
const CENTER_LNG_DEFAULT = 103.819836;
...
class CommonMap extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.mapUrl != this.state.mapUrl) {
      if (this.state.mapUrl.includes(encodeURIComponent('geo_type=district'))) {
    ...
    if (
      ...
      if (this.state.mapUrl.includes(encodeURIComponent('geo_type=asset'))) {
        outer = {
  ...
  _getCentroid(coords) {
    ...
    console.log('_getCentroid',center);
    return center;
  }
    ...
    console.log('_getDistanceFromLatLonInKm',d);
    return d;
  }
  _deg2rad(deg) {
  ...
  _getXYParam(url) {
    console.log('_getXYParam url',url)
    var trim_end = url.indexOf(encodeURIComponent('&radius='));console.log('trim_end',trim_end)
    var trim_first = url.indexOf(encodeURIComponent('&x='));console.log('trim_first',trim_first)
    var trim_result = decodeURIComponent(url.slice(trim_first, trim_end));console.log('trim_first',trim_first)
    var trim_x_first = trim_result.indexOf('&x=') + 3;console.log('trim_first',trim_first)
    var trim_x_last = trim_result.indexOf('&y=');console.log('trim_first',trim_first)
    var x = trim_result.slice(trim_x_first, trim_x_last);console.log('trim_first',trim_first)
    var trim_y = trim_result.indexOf('&y=') + 3;console.log('trim_first',trim_first)
    var y = trim_result.slice(trim_y);console.log('trim_first',trim_first)
    var param = [x, y];
    console.log('_getXYParam',param);
    return param;
  }
  _getDistrict(url) {
    var trim_end = url.indexOf(encodeURIComponent('&bedroom_min='));
    var trim_first = url.indexOf(encodeURIComponent('&district=')) + 14;
    var trim_result = decodeURIComponent(url.slice(trim_first, trim_end));
    console.log('_getDistrict',trim_result);
    return trim_result;
  }
  _getDistance(url){
    ...
    console.log('_getDistance',trim_result)
    return trim_result;
  }
  _mapSearchResultFeedback(title, data) {
    ...
    this._animateRegion(this.state.layer)
    mapURL = this.mapURL;
    if (data.t != 'Districts' && data.t != 'HDB Towns' && data.t.length > 0) {
      this.setState({
        layer: 'asset',
         suggestionBy: 'asset'
      });
      mapURL = this._constructMapURL(data).replace(
        encodeURIComponent('mode=list'),
        encodeURIComponent('mode=map&geo_type=asset')
      );
    } else if (data.t == 'Districts' || data.t == 'HDB Towns') {
      this.setState({
        layer: 'planning',
        suggestionBy: ''
      });
    ...
    this.setState({
      ...
      //longitude: this.state.longitude,
      //latitude: this.state.latitude,
      latitudeDelta: mapURL.includes(encodeURIComponent('geo_type=asset'))
        ? 0.5 / 12
        : mapURL.includes(encodeURIComponent('geo_type=region'))
          ? 0.5
          : 0.5 / 3,
      longitudeDelta: mapURL.includes(encodeURIComponent('geo_type=asset'))
        ? this.state.longitudeDelta
        : mapURL.includes(encodeURIComponent('geo_type=region'))
          ? 0.3
          : 0.3 / 3
    });
    if (mapURL.includes(encodeURIComponent('geo_type=asset'))) {
      SVY21.init();
      ...
      this.setState({
        suggestionBy: 'asset',
        latitude: getLatLonFilter.lat,
        longitude: getLatLonFilter.lon,
        latitudeDelta: 0.5 / 7,
        longitudeDelta: 0.3 / 7
      });
      //this.flag = false;
      this.center = {lat:[],lng:[]}
      this.center.lat.push(parseFloat(getLatLonFilter.lat));
      this.center.lng.push(parseFloat(getLatLonFilter.lon));
    }
    else if(mapURL.includes(encodeURIComponent('geo_type=region'))){
      this.setState({
        latitude: CENTER_LAT_DEFAULT,
        longitude: CENTER_LNG_DEFAULT
      })
  ...
  _handleDistrictFeedBack(){
    if(this.mapURL.includes(encodeURIComponent('geo_type=district'))){
  ...
  _requestMapUrlbyZoom() {
    ...
    if (layer == 'region') {
      ...
      if (this.state.mapUrl.includes(encodeURIComponent('geo_type=region'))) {
  ...
  mappingArea(area) {
    console.log('mappingArea',area);
    if (area && area.listing_count != 0 && !area.real_name) {
  ...
  _getTempData(data, itemIndex) {
    console.log('_getTempData',data[itemIndex]);
  ...
  _getMapData(type) {
    ...
    ////////////////////////// LAYER 3 //////////////////////////////////
    else if (this.state.mapUrl.includes(encodeURIComponent('geo_type=asset'))) {
      if (type == 'markerRegion') {
    ...
    fetch(apiUrl, {
      ...
      .then(responseJson => {
        if (responseJson) {
          ...
          this.setState({
            [stateName]: responseJson.results.data,
            isMeasured: false,
            isShowIcon: true,
            isLoading: false
          });
          ...
          if (this.state.layer == 'region' && this.tempRegion.length == 0) {
            this.tempRegion = responseJson.results.data;
          } else if (
            this.state.layer == 'planning_area' &&
            this.tempPlanningArea.length == 0
          ) {
            this.tempPlanningArea = responseJson.results.data;
          }
        }
      })
      .catch(error => {
  ...
  _getIcon(icon) {
    ...
    if (icon.listing_count.length <= 2 && this.state.layer != 'asset') {
      markerStyle['paddingHorizontal'] = 12;
    } else if (icon.listing_count.length == 3 && this.state.layer != 'asset') {
      markerStyle['paddingHorizontal'] = 10;
    } else if (this.state.layer != 'asset') {
      markerStyle['paddingHorizontal'] = 6;
    } else if (this.state.layer == 'asset' && icon.listing_count.length < 2) {
      markerStyle['paddingHorizontal'] = 10;
    } else if (this.state.layer == 'asset' && icon.listing_count.length == 2) {
  ...
  _constructMapURL(data) {
    let mapURL =
      ...
      encodeURIComponent(
        API_DOMAIN +
          '/index.php?option=com_analytica&task=mapsearch&mode=list&' +
          this._handleParamSearchOption(data) +
          'radius=1000&search_by=&search_by_distance=&search_by_location=&search_by_showmap=true&below_valuation=&map_zoom=&asset_lat=&asset_lng=&page=' +
          '&pageSize=' +
          PAGE_SIZE +
          ''
      );
    console.log('_constructMapURL',mapURL);
    return mapURL;
  }
  ...
  _isShowCallout(item){
    if (this.state.willShowCallout){
      return (['i','l','r','c','h'].includes(this.paramsData['t']) && (this.paramsData['keyword']==item.name))
    }
    return false
  }
  _getCircle(param){
            ...
            if (data != undefined && this.state.isShowIcon) {
              return (
                <MapView.Marker
                  ...
                  onPress={() => {
                    // console.log('MARKED', this.state.markerRegion[index]);
                    if (
                      !this.state.mapUrl.includes(
                        encodeURIComponent('geo_type=asset')
                      )
                      ...
                      this.refs.mapView.animateToRegion({
                        ...
                        latitudeDelta: this.state.mapUrl.includes(
                          encodeURIComponent('geo_type=region')
                        )
                          ? this.state.latitudeDelta / 5
                          : this.state.latitudeDelta / 13,
                        //ZOOM_LEVEL_DEFAULT_DISTRICT,
                        longitudeDelta: this.state.mapUrl.includes(
                          encodeURIComponent('geo_type=region')
                        )
                          ? this.state.latitudeDelta / 5
                          : this.state.latitudeDelta / 13
                ...
                >
                      ...
                      onPress={() => {
                        this.refs.navigationHelper._navigate('ListingResult', {
                          data: {
                            ...
                            a: this.state.markerRegion[index].asset_id,
                          },
                          title: 'CommonMap'
                        });
                      }}
                        ...
                        >
                          ...
                          >
                            ...
                            {this.state.markerRegion[data].top != '' && this.state.markerRegion[data].top? (
                              <Text>{this.state.markerRegion[data].top}. </Text>
                          ...
                          this.state.markerRegion[data].avg_psf != undefined ? (
                            ...
                            >
                              Avg Price: S${this._formatNumber(
                                this.state.markerRegion[data].avg_psf
                              )}{' '}
                              psf
                            </Text>
                  ...
                  {Platform.OS == 'ios' ? (
                    <PinMarker
                      ...
                      imageHeight={40}
                      overlayText={this.state.markerRegion[data].listing_count}
                    />
                  ) : null}
                </MapView.Marker>
              );
