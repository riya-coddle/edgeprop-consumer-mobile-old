...
import styles from './HomeFeaturedDetailsItemStyle.js'
const {height, width} = Dimensions.get('window');
const itemWidth = (width - 26) / 2;

class HomeFeaturedDetailsItem extends Component {
    imagesData = []
    launchData = []
    constructor(props) {
        super(props)
        ...
        return(
            ...
            <TouchableOpacity onPress= {this._onPress} style={styles.sliderContainer}>
              <View style={{ borderRadius: 3, overflow: 'hidden'}}> 
                <ImageBackground
                  source={{uri: this.props.item.field_cover_thumbnail_s_lower}}
                  resizeMode= 'cover'
                  style={styles.propItemImage}
                  >
                  <View style={styles.propOverlay}>
                    <Text allowFontScaling={false} style={styles.discoverTitle}>{this.props.isEditor?this.props.item.title.toUpperCase():this.props.item.title_t.toUpperCase()}</Text>
                  </View>
                </ImageBackground>
              </View>
            </TouchableOpacity>
...
export default HomeFeaturedDetailsItem;
